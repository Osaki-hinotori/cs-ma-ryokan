...
### 会話文脈の参照（必須）
- 返信を作成する前に、必ずNotionの顧客ページや過去のやり取りを参照し、会話の流れに合った文脈の返信を考える
- 以前送った内容・提案内容・相手の反応を踏まえて、一貫性のある返信にする
- テンプレを機械的に当てはめず、相手との関係性やフェーズに応じた対応をする
...
### 提案の基本方針
- **日帰りは基本的におすすめしない**。旅館の魅力は「泊まること」にある（夕食の懐石、朝風呂、静かな夜の温泉など）。日帰りでは体験の半分も伝わらないため、必ず1泊以上の宿泊を提案する
- 顧客が「日帰りで行きたい」と言った場合も、宿泊の価値をやんわり伝えて1泊を推奨する
...
### 旅館リサーチ基準（カタログ外の宿を提案する場合）
顧客の希望エリアがカタログ外の場合、以下の基準でリサーチ・選定する：
- **優先度1**: 海外OTA（Booking.com、Agoda、Expedia等）に未掲載であること。国内OTA（楽天、じゃらん等）には載っていてもOK
- **優先度2**: Googleレビューの星4.3以上

### リサーチ候補リストの表示フォーマット（必須）
旅館リサーチ結果を提示する際は、必ず冒頭に以下の顧客情報ヘッダーを付ける：

```
【リサーチ依頼】
顧客名: ○○○
チャネル: Insta_YRB / WhatsApp / LINE 等
緊急度: 🔥最優先 / 高 / 中 / 低
予算: ¥○○,○○○（1泊1人 or 合計）
人数: ○名
泊数: ○泊○日（例: 1泊2日、2泊3日）
特記事項: タトゥーあり / ベジタリアン / 子供あり / 車椅子 等
```
...
- **会話内容の転記**: 顧客から受信したメッセージ・こちらから送信したメッセージは、両方ともNotion顧客ページに必ず転記する。転記漏れ禁止
- **随時更新ルール（絶対厳守）**:
  - 返信ドラフトにOKが出たら、**即座に**Notion CS管理DBを更新する（後回し禁止）
  - 新規顧客 → ページ作成（タイトル形式に従う）
  - 既存顧客 → 会話ログ追記 + CSステータス・フェーズ更新
  - 1件の対応完了ごとにNotionを更新してから次の対応に移る
  - 漏れは一切許容しない
- **希望エリア選択肢の随時追加**: Notion CS管理DBの「希望エリア」multi_selectに該当する選択肢がない場合、`update-data-source`で新しい選択肢を追加してから値をセットする。「その他」でごまかさない
...
```
JoJにDM → JoJには返信しない → YRBからフォロー → YRBから初回テンプレートを送信 → 以降YRBでやり取り
```
- **JoJへの返信は不要**（リダイレクトメッセージも送らない）
- YRBからフォロー＆テンプレート送信のみ
- **JY発生の経緯**: JoJの投稿コメントに「YoheiにDMで"JOJ"と送ると旅行相談に乗ってくれる」という案内があり、それ経由でJoJにDMが来る
...
## 11. ライティング（ブログ・SNS投稿・LP作成）

### 基本トーン
- Yoheiの人柄が伝わる一人称スタイル（「僕が実際に泊まって…」）
- 観光ガイド風ではなく、友達に「ここヤバいよ」と教える感覚
- AI感排除。テンプレ的な表現（「魅力をご紹介します」等）は禁止

### ブログ記事ルール
- タイトル: 検索意図を意識しつつクリックしたくなる形（例：「鬼滅の刃の無限城、実在した。」）
- 構成: リード文（体験ベース）→ 旅館の魅力 → アクセス → 予約導線
- 写真挿入ポイントを `[PHOTO: 説明]` で明示
- CTAは必ず `ryokan-book.com` の該当ページへ誘導
- SEOキーワードは自然に織り込む。キーワード詰め込み禁止

### Instagram投稿ルール
- **キャプション**: 1行目にフック（スクロール止める一言）。改行多め。ハッシュタグは10〜15個
- **ストーリーズ**: 短文＋絵文字。投票・質問スタンプ活用推奨
- **リール台本**: フック（0-3秒）→ 本題（3-15秒）→ CTA（最後3秒）の構成
- アカウント別:
  - **@japan_of_japan_**: 日本の魅力を海外に発信。英語メイン
  - **@yohei_ryokanbook**: 旅館特化。パーソナルな体験共有

### LP（ランディングページ）作成ルール
- Notion公開ページで作成
- 構成: ヘッドライン → 旅館の写真＋説明 → 料金目安 → 「Book with Yohei」CTA
- 顧客別カスタムLP（例：「El Maje Oar — Kogetsu & Kiyonoya Ryokan Guide」）も同じ構成
- 料金は必ず売値で記載
...
### 翻訳品質基準
- **自然さ最優先**: 直訳禁止。その言語のネイティブが書いたように
- **固有名詞**: 旅館名は日本語表記＋ローマ字併記（例：大川荘 Ookawaso）
- **通貨**: 基本は円（¥）表記。英語圏にはUSD換算も併記可（例：~$200/night）
- **敬語レベル**: 言語ごとに調整
  - 英語: フレンドリー（Hi! / Hey!）
  - 中国語: やや丁寧だが堅すぎない（你好！/ 感謝你）
  - 日本語: 顧客向けは丁寧語、社内はカジュアル

### 翻訳チェックリスト（必須）
- [ ] AI的表現が残っていないか（「I'd be happy to」等）
- [ ] 金額は売値になっているか
- [ ] URLはryokan-book.comのものか
- [ ] 旅館名の表記が統一されているか
- [ ] 文化的に不適切な表現がないか
...
- 台湾向け: 「你好」「謝謝」（繁体字）
- 中国大陸向け: 「你好」「谢谢」（簡体字）
- 判別できない場合はまず繁体字で（Ryokanbookの顧客は台湾・香港が多い）

---

## 13. 予約オペレーション（見積もり・請求書）

### 見積もり作成ルール
- **顧客向け見積もり**: 売値（卸値×約1.2）で作成
- **宿向け依頼**: 卸値で記載
- フォーマット:
...
### 請求・決済フロー
1. 見積もり確定 → Stripe決済リンク作成
2. 決済リンクを顧客に送付
3. 決済完了 → Notion CSステータスを「決済完了」に更新
4. 宿に予約確定連絡（卸値で）

### 予約確認メールフォーマット

```
Subject: Booking Confirmation - [旅館名] | Ryokanbook
...
If you need anything before your stay, just let me know!
...
| 変更内容 | 5日前まで | 4日前以降 |
|---------|---------|---------|
| 宿泊日の変更（同じ宿） | 無料 | キャンセル扱い |
| 人数のみ変更 | 無料 | 要相談 |
| プランの変更 | 無料 | 要相談 |

- **同じ宿泊施設の予約変更は5日前まで無料対応**
- **違う宿への予約変更は「キャンセル扱い」**
※旅館ごとに異なる場合あり。必ず個別確認
...
### KPI定義
- **CVR（コンバージョン率）**: 決済完了数 ÷ 全リード数
- **GMV（総取扱高）**: 売値ベースの決済合計
- **平均リードタイム**: 初回DM〜決済完了の平均日数
- **平均顧客単価**: GMV ÷ 決済完了顧客数
- **チャネル別CVR**: チャネルごとのコンバージョン率

### 分析依頼時のルール
- Notion CS管理DBからデータを取得して分析
- グラフは必要に応じてテキストベースで表現
- 個人情報（氏名・連絡先）はレポートに含めない（匿名化）
- 週次・月次で定期レポート可能
